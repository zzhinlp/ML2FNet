...
from unetplusplus2 import UnetPlusPlus
...
class DocREModel(nn.Module):
    def __init__(self, config, args, model, emb_size=768, block_size=64, num_labels=-1):
        ...
        self.liner = nn.Linear(config.hidden_size, 3)
        ...
        # self.segmentation_net = VNet()
        self.segmentation_net = UnetPlusPlus(num_classes=256,deep_supervision=False)
        # self.segmentation_net = DeepLabV3Plus(n_classes=64,
        #                                         n_blocks=[3, 4, 23, 3],
    ...
    def forward(self,
        ...
        if self.channel_type == 'context-based':
            feature_map = self.get_channel_map(sequence_output, entity_as) # [4, 42, 42, 768]
            ##print('feature_map:', feature_map.shape)
            # attn_input = feature_map.permute(0, 3, 1, 2).contiguous() # [4, 3, 42, 42]
            attn_input = self.liner(feature_map).permute(0, 3, 1, 2).contiguous() # [4, 3, 42, 42]
        ...
        # attn_input = torch.unsqueeze(attn_input,dim=1)
        # # attn_input = attn_input.permute(0, 1,4,2, 3)  # [4, 42, 42, 256]
        attn_map = self.segmentation_net(attn_input) # [4, 42, 42, 256]
        # attn_map = attn_map.permute(0,2,3,1)
        h_t = self.get_ht (attn_map, hts)#attn_map是u型网络之后的矩阵，hts是所有的实体对，包括正例和负例，如果42个实体，hts所有实体对就是42*41
